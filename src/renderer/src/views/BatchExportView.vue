...
<template>
  <div class="flex flex-col h-full p-6 overflow-y-auto">
    <h2 class="font-display text-xl text-accent mb-6">{{ $t('batchExport.title') }}</h2>
    ...
    <div class="max-w-2xl space-y-6">
      ...
      <div class="bg-bg-secondary rounded-xl border border-white/5 p-4">
        ...
        <button @click="selectOutputDir"
                class="w-full text-left px-3 py-2 text-sm rounded-lg bg-bg-tertiary border border-white/5 text-text-secondary hover:bg-bg-hover hover:text-text-primary transition-colors">
      ...
      <div class="bg-bg-secondary rounded-xl border border-white/5 p-4">
        ...
        <div class="space-y-4">
          <div>
            ...
            <div class="flex gap-2">
              <button v-for="f in formats" :key="f.value"
                      ...
                      :class="format === f.value ? 'bg-accent-dim text-accent' : 'bg-bg-tertiary text-text-secondary hover:bg-bg-hover'">
                {{ f.label }}
              </button>
            </div>
          </div>
          ...
          <div class="flex gap-4">
            <div>
              ...
              <input v-model.number="maxWidth" type="number" placeholder="不限"
                     class="w-32 bg-bg-tertiary border border-white/5 rounded-lg px-3 py-1.5 text-sm text-text-primary outline-none focus:border-accent/30" />
            </div>
            <div>
          ...
          <label class="flex items-center gap-2 cursor-pointer">
            <input v-model="keepExif" type="checkbox" class="accent-accent" />
      ...
      <div v-if="processing" class="mb-2">
        <div class="text-xs text-text-muted mb-1">{{ progress.current }} / {{ progress.total }}</div>
