...
<template>
  <div class="flex flex-col h-full p-6 overflow-y-auto">
    <h2 class="font-hand text-xl text-fuji-warm mb-6">{{ $t('batchRename.title') }}</h2>
    ...
    <div class="max-w-2xl space-y-6">
      ...
      <div class="bg-bg-secondary rounded-xl border border-border-subtle p-4">
        ...
        <input v-model="template" type="text"
               class="w-full bg-bg-tertiary border border-border-subtle rounded-lg px-3 py-2 text-sm text-text-primary font-mono outline-none focus:border-fuji-warm/30 mb-3"
               placeholder="{date}_{camera}_{seq:3}" />
        <div class="flex flex-wrap gap-2">
          <button v-for="p in placeholders" :key="p.key"
                  @click="template += p.key"
                  class="px-2 py-1 text-xs rounded bg-bg-hover text-text-secondary hover:text-fuji-warm transition-colors font-mono"
                  :title="p.desc">
            {{ p.key }}
          </button>
        </div>
      ...
      <div class="bg-bg-secondary rounded-xl border border-border-subtle p-4">
        ...
        <div class="flex items-center gap-3">
          ...
          <input v-model.number="startSeq" type="number" min="1"
                 class="w-20 bg-bg-tertiary border border-border-subtle rounded-lg px-3 py-1.5 text-sm text-text-primary outline-none focus:border-fuji-warm/30" />
        </div>
      </div>
      ...
      <div v-if="step === 'preview'" class="space-y-4">
        <div v-if="hasConflicts" class="bg-fuji-red/10 border border-fuji-red/20 rounded-lg px-4 py-3">
          <span class="text-sm text-fuji-red">⚠ {{ $t('rename.conflictWarning') }}</span>
        </div>
        <div class="bg-bg-secondary rounded-xl border border-border-subtle p-4">
          ...
          <div class="space-y-1 max-h-64 overflow-y-auto">
            <div v-for="(item, i) in previewItems" :key="i"
                 ...
                 :class="item.conflict ? 'text-fuji-red' : 'text-text-muted'">
              <span class="text-text-secondary flex-1 min-w-0 truncate">{{ item.oldName }}</span>
              <span class="text-fuji-warm shrink-0">→</span>
              <span class="flex-1 min-w-0 truncate" :class="item.conflict ? 'text-fuji-red' : ''">{{ item.newName }}</span>
              <span v-if="item.conflict" class="text-fuji-red text-[10px] shrink-0">{{ $t('rename.conflict') }}</span>
            </div>
          </div>
        </div>
      ...
      <!-- 步骤 3: 完成结果 -->
      <div v-if="step === 'done' && results.length > 0" class="bg-bg-secondary rounded-xl border border-border-subtle p-4">
        ...
        <div class="space-y-1 max-h-64 overflow-y-auto">
          <div v-for="(r, i) in results" :key="i" class="text-xs text-text-muted truncate">
            ...
            <span class="text-fuji-warm mx-2">→</span>
