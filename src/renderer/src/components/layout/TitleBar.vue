...
<template>
  <div class="h-10 bg-bg-primary border-b border-border-subtle flex items-center px-4 select-none app-drag"
       :class="isMac ? 'pl-20' : ''">
    <div class="flex items-baseline gap-1.5">
      <span class="font-mono text-[11px] font-bold text-fuji-red tracking-[0.15em]">FUJI</span>
      <span class="font-sans text-sm font-semibold text-text-primary">{{ t('app.name') }}</span>
    </div>
    <div class="ml-auto flex gap-1 app-no-drag">
      <button class="w-7 h-7 flex items-center justify-center rounded-md text-fuji-red hover:bg-fuji-red-dim transition-colors"
              @click="handleImport"
      ...
      <template v-if="!isMac">
        ...
        <button class="w-8 h-7 flex items-center justify-center text-text-secondary hover:bg-fuji-red/80 hover:text-white transition-colors"
                @click="close"
